...
predict_model3 <- function(model_environment) {
  function(par){
...
llk_model3 <- function(model_environment) { 
  function(par){
    if (!is.null(names(par))) for (i in seq_along(par))  assign(names(par)[i], par[i], envir = model_environment)
    else for (i in seq_along(model_environment[["parameter"]]))  assign(model_environment[["parameter"]][i], par[i], envir = model_environment)
...
gradient_model_env3 <- function(model_environment) {
  function(par){   
    if (!is.null(names(par))) for (i in seq_along(par))  assign(names(par)[i], par[i], envir = model_environment)
    else for (i in seq_along(model_environment[["parameter"]]))  assign(model_environment[["parameter"]][i], par[i], envir = model_environment)    
...
hessian_model_env3 <- function(model_environment) {
  function(par){
    if (!is.null(names(par))) for (i in seq_along(par))  assign(names(par)[i], par[i], envir = model_environment)
    else for (i in seq_along(model_environment[["parameter"]]))  assign(model_environment[["parameter"]][i], par[i], envir = model_environment)    
...
gradient_model_env2 <- function(model_environment) {
  function(par, data = NULL, ...){   
    ...
    if (is.null(data)) {
      data  <- rep(1, sum(model_environment[["n_item_type"]]))/rep(model_environment[["n_item_type"]], times = model_environment[["n_item_type"]])
    }
